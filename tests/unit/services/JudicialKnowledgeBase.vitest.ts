/**
 * JudicialKnowledgeBase Vitest Tests
 * 
 * These tests use Vitest which has native ESM support, solving the Jest + LangChain compatibility issues.
 * 
 * Prerequisites:
 * 1. Start Chroma: docker-compose up -d chroma
...
// These tests require Chroma to be running
// To skip: npm run test:vitest -- --grep -v "Integration"
describe('JudicialKnowledgeBase - Integration Tests', () => {
    ...
    describe('Question Answering', () => {
        ...
        it('should throw error when querying before initialization', async () => {
            ...
            // Act & Assert
            await expect(
                uninitializedKB.query('test question')
            ).rejects.toThrow('Knowledge base not initialized');
        });
    });
